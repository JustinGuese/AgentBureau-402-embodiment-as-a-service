...
def create_invoice_payment_flow(input_str: str):
    """Creates an invoice. Input should be a JSON string with 'customer_details' and 'line_items'."""
    try:
...
tools = [
    Tool(
        name="CreateInvoice",
        func=create_invoice_payment_flow,
        description="Useful for generating professional invoices. Input: JSON string with customer_details object and line_items array."
    )
]
...
if __name__ == "__main__":
    ...
    prompt = """Create an invoice for Acme Corp (billing@acme.com, 123 Business Way, London) 
    for 10 units of AI Consulting at 150.00 each."""
    agent.run(prompt)
